	...
	if (isset($_SESSION["codice_utente"]) && isset($_SESSION["ente"])) {
		...
		if (isset($_GET["codice"])&&isset($_GET["codice_bando"])) {
					...
					if ($risultato->rowCount() > 0) {
						...
						$strsql = "SELECT b_cpv.* FROM b_cpv JOIN r_cpv_bandi_albo ON b_cpv.codice = r_cpv_bandi_albo.codice WHERE r_cpv_bandi_albo.codice_bando = :codice ORDER BY codice";
						...
						$strsql = "UPDATE r_partecipanti_albo SET visto = 'S' WHERE codice_bando = :codice_bando AND codice_operatore = :codice_operatore ";
						$risultato = $pdo->bindAndExec($strsql,$bind);
						...
						$risultato = $pdo->bindAndExec($strsql,$bind);
	                      ...
	                      <div id="tabs">
	                          ...
	                          <li><a href="#allegati">Modulistica</a></li>
	  ...
	  <td class="etichetta">Data emissione</td><td><? echo mysql2date($record_operatore["data_emissione_certificato"]) ?></td></tr>
	                     ...
	                     <div id="committenti">
 	  						...
 	  						?>
	 	                         ...
	 	                         <? if (isset($risultato_soa_fatturato) && $risultato_soa_fatturato->rowCount() > 0) {
	 	  						   			while ($soa_fatt = $risultato_soa_fatturato->fetch(PDO::FETCH_ASSOC)) {
	 	  										include($root."/albo/soa_fatturato/view.php");
	 	  									}
	 	  					   		}
	 	  						?>
	 	                          </tbody>
	                      ...
	                      <?
	                        ...
	                        $sql = "SELECT * FROM b_modulistica_albo WHERE codice_bando = :codice_bando ORDER BY attivo DESC, codice";
	                        ...
	                        $moduli = [];
													...
													if (!empty($record_bando["jsonQuestionario"])) {
														...
														$moduloQuestionario["titolo"] = "Questionario generato dal sistema";
														$moduloQuestionario["obbligatorio"] = "S";
														$moduloQuestionario["attivo"] = "S";
	                          ...
	                          foreach ($moduli AS $record_modulo) {
	                            ...
	                            $strsql  = "SELECT * FROM b_allegati_albo WHERE ";
	                            ...
	                            if($ris_allegato->rowCount()>0) {
	                              ...
	                              $p7m = new P7Manager($config["arch_folder"] . "/allegati_albo/" . $allegato["codice_operatore"] . "/" . $allegato["riferimento"]);
																if (!empty($p7m->fileContent)) {
		                              ...
		                              foreach ($certificati AS $esito) {
		                                ...
		                                $note .=  "</li></ul>";
															    ...
															    $mime_type = $file_info->buffer(file_get_contents($config["arch_folder"] . "/allegati_albo/" . $allegato["codice_operatore"] . "/" . $allegato["riferimento"]));
	                            ...
	                            ?>
	                          	...
	                          	<? }
													...
													<div id="comunicazioni">
														<?
															$strsql = "SELECT b_comunicazioni.*, r_comunicazioni_utenti.sync, r_comunicazioni_utenti.codice as codice_relazione, r_comunicazioni_utenti.protocollo, r_comunicazioni_utenti.data_protocollo FROM b_comunicazioni JOIN r_comunicazioni_utenti ON b_comunicazioni.codice =
															r_comunicazioni_utenti.codice_comunicazione
																				 WHERE  b_comunicazioni.sezione = 'albo' AND b_comunicazioni.codice_gara = :codice_bando AND
										...
										<? if ($record_partecipante["ammesso"] != 'S' || $record_partecipante["valutato"] == 'N') { ?>
													<form action="/albo_fornitori/abilitazione/abilita.php" method="post">
	                              ...
	                              <input type="submit" class="submit_big" style="background-color:#0C0" value="Abilita">
										...
										<? if (($record_partecipante["ammesso"] != 'N' && $record_partecipante["valutato"] == "S") || $record_partecipante["valutato"] == "N") { ?>
														...
														<div id="form_dialog" style="display:none">
																<form action="/albo_fornitori/abilitazione/respingi.php" rel="validate" method="post">
																			<input type="hidden" name="codice_operatore" value="<?= $codice ?>">
																			<input type="hidden" name="codice_bando" value="<?= $record_bando["codice"] ?>">
										...
										<? } ?>
														<script>
															function ritorna() {
																window.location.href = "/albo_fornitori/partecipanti/index.php?codice=<? echo $record_bando["codice"] ?>";
															}
