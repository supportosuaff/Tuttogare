	...
	if (isset($_GET["codice_bando"]) && isset($_GET["key"])) {
		...
		if (in_array($_GET["key"], $chiavi)) {
			...
			if ($risultato->rowCount() > 0) {
				...
				if (!empty($_SESSION["exportElencoPartecipanti"]["albo"][$bando["codice"]][$key])) {
					$data = $_SESSION["exportElencoPartecipanti"]["albo"][$bando["codice"]][$key];
					header("Content-type: text/csv");
					header("Content-Disposition: attachment; filename=file.csv");
					header("Pragma: no-cache");
					header("Expires: 0");
					echo "\"#\";\"Feedback\";\"Codice Fiscale Impresa\";\"Ragione sociale\";\"Data richiesta\"";
					...
					foreach ($data AS $record_partecipante) {
						$i++;
						echo "\""; echo $i; echo "\";"; 
						echo "\""; echo $record_partecipante["feedback"]; echo "\";";
