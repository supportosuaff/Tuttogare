	...
	if ((isset($_GET["codice"]) || isset($_GET["cod"]))) {
			...
			die();
				...
				if ($risultato->rowCount() > 0) {
					...
					$sql = "SELECT * FROM b_allegati WHERE codice_gara = :codice_gara AND sezione = 'albo' AND online = 'S' ORDER BY cartella, codice";
					$ris_allegati = $pdo->bindAndExec($sql,$bind);
					$sql = "SELECT * FROM b_allegati WHERE codice_gara = :codice_gara AND sezione = 'albo' AND online = 'N' ORDER BY cartella, codice";
