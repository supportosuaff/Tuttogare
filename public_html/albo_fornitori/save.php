<?
	...
	if (!$edit) {
		...
		if (isset($_POST["operazione"])) {
			if (!empty($_POST["bando"]["jsonQuestionario"])) {
				$json = json_decode($_POST["bando"]["jsonQuestionario"]);
				if (empty($json)) {
					?>
					alert("JSON questionario errato. Nessun salvataggio effettuato");
					<?
					die();
				}
			}
			if ($_POST["operazione"]=="INSERT") {
				...
				$sql = "SELECT max(id) as id FROM b_bandi_albo WHERE codice_gestore = :codice_ente GROUP BY codice_gestore ";
			...
			$_POST["bando"]["manifestazione_interesse"] = ($_POST["bando"]["tipologia"] == "M") ? "S" : "N";
			...
			if (isset($_POST["cpv"])) {
						if ($_POST["cpv"] != "")  {
							...
							$strsql = "DELETE FROM r_cpv_bandi_albo WHERE codice_bando = :codice_bando ";
							...
							foreach($array_cpv as $cpv) {
								if ($cpv != "") {
									...
									$salva->debug = false;
									$salva->codop = $_SESSION["codice_utente"];
									$salva->nome_tabella = "r_cpv_bandi_albo";
			...
			$href = "/albo_fornitori/pannello.php?codice=" . $codice_bando;
