...
if (!$edit) {
	...
	if ($risultato->rowCount() > 0) {
		...
		if ($risultato->rowCount() > 0) {
			...
			$strsql = "UPDATE r_partecipanti_albo SET valutato = 'S', ammesso = 'N' WHERE codice_operatore = :codice_operatore AND codice_bando = :codice_bando";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			scrivilog("r_partecipanti_albo","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);

			if ($record_bando["manifestazione_interesse"] == "N") {
				$oggetto = "Respinta ammissione all'elenco dei fornitori: " . $record_bando["oggetto"];
			} else if ($record_bando["manifestazione_interesse"] == "S") {
				$oggetto = "Respinta ammissione all'indagine di mercato: " . $record_bando["oggetto"];
			}

			$corpo = "Si informa che a seguito di valutazione, la S.V. non &egrave; stata abilitata ";
			if ($record_bando["manifestazione_interesse"] == "N") {
			...
			$href = str_replace(' ',"-",$href);
			transferToAlboCommissari("DELETE",$operatore["codice"],$record_bando["codice"]);
