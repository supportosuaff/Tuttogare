					...
					<?
						if (is_operatore()) {
								...
								if ($ris->rowCount() > 0) {
									...
									if ($partecipante["conferma"] == "S") $partecipato = true;
								}
								if (!empty($record_bando["jsonQuestionario"])) {

									$url = "/albo_fornitori/questionario/edit.php?cod={$record_bando["codice"]}";
									$moduloQuestionario = [];
									...
									if (empty($partecipante["hashQuestionario"])) {
										...
										?>
										<a class="submit_big" style="background-color:#0CF" href="<?= $url ?>">Domanda di iscrizione</a>
										<?
										 	if (!empty($partecipante["hashQuestionario"])) { ?>
								...
								<form id="modulo" rel="validate" method="post" target="_self" action="save_modulo.php">
									...
									<? } ?>
										<table width="100%">
											<thead>
												<tr><td><?= traduci("Modulo") ?></td><td><?= traduci("Obbligatorio") ?></td><td><?= traduci("Allegato") ?></td></tr></thead>
											<?
												$bind = array();
												$bind[":codice_bando"] = $record_bando["codice"];
												$sql = "SELECT * FROM b_modulistica_albo WHERE codice_bando = :codice_bando AND attivo = 'S' ORDER BY codice";
													...
													foreach ($moduli AS $record_modulo) {
														...
														<tr>
															...
															<td width="25%">
																...
																<div id="modulistica_<? echo $record_modulo["codice"] ?>" rel="<? echo $record_modulo["codice"] ?>" class="scegli_file" style="float:right"><span class="fa fa-folder-open"></span></div>
																<div class="clear"></div>
								...
								<input class="submit_big" type="submit" value="<?= traduci("Salva") ?>" onclick="save()">
								<?
									if (!$partecipato) {
										?>
										<input class="submit_big" type="submit" value="<?= traduci("Salva ed invia") ?>" style="background-color: #0C0" onclick="save_invia();">
										<?
									} else {
										?>
										<a id="button_revoca_<? echo $partecipante["codice"] ?>" href="#" onClick="elimina('<? echo $partecipante["codice"] ?>','albo_fornitori/abilitazione');" class="submit_big" style="background-color:#C30" title="<?= traduci("Revoca partecipazione") ?>"><?= traduci("Revoca partecipazione") ?></a>
										<?
									}
								?>
							</form>
