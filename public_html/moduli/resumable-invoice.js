    ...
    window.ResumableUploader = function(browseTarget) {
     ...
     this.newResumable = function(){
       ...
       this.resumable.on('fileAdded', function(file){
		   var allReg = /\.(xml|XML)$/;	// espressione regolare per il controllo dei file allegati
		   if (allReg.test(file.fileName)) {
				$("#filechunk").val(file.fileName);
        $this.resumable.upload();
		   } else {
			  jalert("<div style=\"text-align:center\">Impossibile caricaricare il file<br><br><strong>" + file.fileName + "</strong></br><br>La tipologia pu&ograve; solo essere: XML</div>");
