...
<table class="bordered bigger">
		...
		<td colspan="4">
			<?
				...
				if(!empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["CONTRACTOR"])) {
					...
					if(!$ajax) { include 'forms/2_0_9/common/ADDR-S5.php'; } else { include '../common/ADDR-S5.php'; }
					$contractor++;
				} else {
					$keys = '[AWARD_CONTRACT][ITEM_'.$item.'][AWARDED_CONTRACT][CONTRACTOR][ITEM_'.$contractor.'][ADDRESS_CONTRACTOR]';
					if(!$ajax) { include 'forms/2_0_9/common/ADDR-S5.php'; } else { include '../common/ADDR-S5.php'; }
	...
	<tr>
		...
		// 		$address_item = $contractor;
		// 		$keys = '[AWARD_CONTRACT][ITEM_'.$item.'][AWARDED_CONTRACT][CONTRACTOR][ITEM_'.$contractor.'][ADDRESS_CONTRACTOR]';
		// 		if(!$ajax) { include 'forms/2_0_9/common/ADDR-S5-f03.php'; } else { include '../common/ADDR-S5-f03.php'; }
		// 		$contractor++;
		// 	}
		// }
		?></td>
	...
	<tr>
		...
		<td colspan="2">
			...
			<input type="text" title="Valore Totale" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_TOTAL][val]" value="<?= $valore_totale ?>" id="valore_totale_del_contaratto_item_<?= $item ?>_duration" rel="S;1;0;2D">
		</td>
	</tr>
	<tr>
		<td class="etichetta" colspan="2">
		...
		<td colspan="2">
			<?
			$val_revenue = "";
			if(!empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["VAL_REVENUE"]["val"])) {
				$val_revenue = $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["VAL_REVENUE"]["val"];
			}
			?>
			<input type="hidden" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_REVENUE][ATTRIBUTE][CURRENCY]" value="EUR">
			<input type="text" title="Entrate derivanti dal pagamento, da parte degli utenti, di tariffe e multe" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_REVENUE][val]" value="<?= $val_revenue ?>" id="valore_totale_del_contaratto_item_<?= $item ?>_duration" rel="N;1;0;2D">
		</td>
	</tr>
	<tr>
		<td class="etichetta" colspan="2">
		...
		<td colspan="2">
			<?
			$val_price_payment = "";
			if(!empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["VAL_PRICE_PAYMENT"]["val"])) {
				$val_price_payment = $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["VAL_PRICE_PAYMENT"]["val"];
			}
			?>
			<input type="hidden" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_PRICE_PAYMENT][ATTRIBUTE][CURRENCY]" value="EUR">
			<input type="text" title="Premi, pagamenti o altri vantaggi finanziari" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_PRICE_PAYMENT][val]" value="<?= $val_price_payment ?>" id="valore_totale_del_contaratto_item_<?= $item ?>_duration" rel="N;1;0;2D">
		</td>
	</tr>
	<tr>
		<td class="etichetta" colspan="4">
