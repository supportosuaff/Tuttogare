...
<tr>
	...
	<td>
		<input type="text" id="qs_date_end_<?= $item ?>" name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][QS][DATE_END]" class="datepick" title="Data di Inizio" rel="S;0;0;D" <?= $radio_as_select_for_qs_duration != "START_END_ITEM_TO_IGNORE" ? 'disabled="disabled"' : null ?> value="<?= !empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["QS"]["DATE_END"]) ? $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["QS"]["DATE_END"] : null ?>">
	</td>
</tr>
<tr>
	<td colspan="4">
		<label>
			<input name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][QS][RENEWAL]" type="checkbox" <?= !empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["QS"]["RENEWAL_DESCR"]) ? 'checked="checked"' : null ?> onchange="toggle_field($(this),'#renewal_descr_<?= $item ?>')" id="justification_checkbox" rel="">
