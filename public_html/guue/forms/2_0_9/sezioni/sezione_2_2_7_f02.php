	...
	<td colspan="4">
		<table class="bordered valida" title="Durata del contratto" rel="<?= isRequired("durata_del_contratto") ?>;0;0;checked;group_validate">
			<tbody>
				...
				<tr>
					<td colspan="4" class="etichetta">
						Il contratto di appalto &egrave; oggetto di rinnovo?
					</td>
				</tr>
				<tr>
					<td class="etichetta" colspan="4">
						<label style="font-size: 14px;">
							...
							if(!empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["radio_as_select_for_renewal"])) {
								$radio_as_select_for_renewal = $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["radio_as_select_for_renewal"];
							}
							?>
							<script type="text/javascript">
								var lot_<?= $item ?>_renewal_option = {
									'RENEWAL' : [
										'enable_field',
										'',
										[],
										'item_<?= $item ?>_renewal_descr'
									]
								};
							</script>
							<select name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][radio_as_select_for_renewal]" title="Rinnovo del Contratto" rel="<?= isRequired("radio_as_select_for_renewal") ?>;1;0;A" onChange="add_extra_info($(this).val(), lot_<?= $item ?>_renewal_option)">
								<option value="">Seleziona..</option>
								<option <?= $radio_as_select_for_renewal == "RENEWAL" ? 'selected="selected"' : null ?> value="RENEWAL">Si</option>
								<option <?= $radio_as_select_for_renewal == "NO_RENEWAL" ? 'selected="selected"' : null ?> value="NO_RENEWAL">No</option>
							</select>
						</label>
					</td>
				</tr>
				<tr>
					<td colspan="4">
						<textarea id="item_<?= $item ?>_renewal_descr" name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][RENEWAL_DESCR]" rel="S;0;400;A" title="Descrizione dei rinnovi" class="ckeditor_simple" <?= $radio_as_select_for_renewal != "RENEWAL" ? 'disabled="disabled"' : null ?>>
							<?= ($radio_as_select_for_renewal == "RENEWAL" && !empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL_DESCR"])) ? $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL_DESCR"] : null ?>
						</textarea>
					</td>
				</tr>
			</tbody>
