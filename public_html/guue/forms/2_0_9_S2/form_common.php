<?
	...
	if(empty($_SESSION["guue"]["post_form"])) {
		if(!empty($_SESSION["guue"]["codice_gara"])) {
			...
			$guue["CONTRACTING_BODY"]["URL_DOCUMENT"] = $config["protocollo"].$_SERVER["SERVER_NAME"]."/gare/id".$_SESSION["guue"]["codice_gara"]."-dettagli";
			...
			if($ris->rowCount() > 0) {
				...
				if($rec["online"] == "S") {
					...
					$guue["CONTRACTING_BODY"]["URL_PARTICIPATION"] = $config["protocollo"].$_SERVER["SERVER_NAME"]."/gare/id".$_SESSION["guue"]["codice_gara"]."-dettagli";
				}
			}
