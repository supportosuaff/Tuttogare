<?
	if (!empty($_POST["id"])) {
		...
		if(!empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC_CRITERION"]) && is_array($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC_CRITERION"])) {
			foreach ($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC_CRITERION"] as $criterio) {
				include 'criteri_di_aggiudicazione_ac_criterion.php';
				$i++;
				$id = "i_".$i;
			}
