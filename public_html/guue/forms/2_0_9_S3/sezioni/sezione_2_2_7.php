				...
				<tr>
					<td class="etichetta" colspan="4">
						<label style="font-size: 14px;">
							<input type="checkbox" name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][RENEWAL]" <?= !empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL"]) ? 'checked="checked"' : null ?> onChange="toggle_field($(this), '#item_<?= $item ?>_renewal_descr')"> Il contratto di appalto &egrave; oggetto di rinnovo
						</label>
					</td>
				</tr>
				<tr>
					<td colspan="4">
						<textarea id="item_<?= $item ?>_renewal_descr" name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][RENEWAL_DESCR]" rel="S;0;400;A" title="Descrizione dei rinnovi" class="ckeditor_simple" <?= empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL"]) ? 'disabled="disabled"' : null ?>>
							<?= (!empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL"]) && !empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL_DESCR"])) ? $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["RENEWAL_DESCR"] : null ?>
						</textarea>
					</td>
				</tr>
			</tbody>
