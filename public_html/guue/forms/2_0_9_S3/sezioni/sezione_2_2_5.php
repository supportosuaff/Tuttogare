...
<tr>
	<td colspan="4">
		...
		if(!empty($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC"]["radio_as_select_for_award_criteria_doc"])) {
			$award_criteria_item_to_ignore = $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC"]["radio_as_select_for_award_criteria_doc"] == "AWARD_CRITERIA_ITEM_TO_IGNORE" ? TRUE : FALSE;
			$ac_procurement_doc = $guue["OBJECT_CONTRACT"]["OBJECT_DESCR"]["ITEM_".$item]["AC"]["radio_as_select_for_award_criteria_doc"] == "AC_PROCUREMENT_DOC" ? TRUE : FALSE;
		}
		?>
		<select name="guue[OBJECT_CONTRACT][OBJECT_DESCR][ITEM_<?= $item ?>][AC][radio_as_select_for_award_criteria_doc]" rel="<?= isRequired("OBJECT_CONTRACT-radio_as_select_for_award_criteria_doc") ?>;1;0;A" title="Criteri di aggiudicazione" onchange="add_extra_info($(this).val(), award_criteria_doc_option_<?= $item ?>);add_extra_info($(this).val(), award_criteria_doc_enable_option_<?= $item ?>)">
			<option value="">Seleziona..</option>
			<option <?= $award_criteria_item_to_ignore ? 'selected="selected"' : null ?> value="AWARD_CRITERIA_ITEM_TO_IGNORE">I criteri indicati di seguito:</option>
