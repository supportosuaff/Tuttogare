...
<table class="bordered bigger">
	...
	<tr>
		...
		<td width="150px">
			<input type="text" title="Numero di offerte" rel="S;1;0;N" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][NB_TENDERS_RECEIVED]" value="<?= !empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED"]) ? $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED"] : null ?>">
		</td>
		<td class="etichetta">Numero di offerte ricevute da PMI:</td>
		<td width="150px">
			<input type="text" title="Numero di offerte" rel="N;1;0;N" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][NB_TENDERS_RECEIVED_SME]" value="<?= !empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_SME"]) ? $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_SME"] : null ?>">
		</td>
	</tr>
	<tr>
		<td class="etichetta">Numero di offerte ricevute da offerenti provenienti da altri Stati membri dell&#39;UE:</td>
		<td>
			<input type="text" title="Numero di offerte" rel="N;1;0;N" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][NB_TENDERS_RECEIVED_OTHER_EU]" value="<?= !empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_OTHER_EU"]) ? $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_OTHER_EU"] : null ?>">
		</td>
		<td class="etichetta">Numero di offerte ricevute dagli offerenti provenienti da Stati non membri dell&#39;UE:</td>
		<td>
			<input type="text" title="Numero di offerte" rel="N;1;0;N" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][NB_TENDERS_RECEIVED_NON_EU]" value="<?= !empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_NON_EU"]) ? $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_NON_EU"] : null ?>">
		</td>
	</tr>
	<tr>
		<td class="etichetta">Numero di offerte pervenute per via elettronica:</td>
		<td>
			<input type="text" title="Numero di offerte" rel="N;1;0;N" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][NB_TENDERS_RECEIVED_EMEANS]" value="<?= !empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_EMEANS"]) ? $guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["NB_TENDERS_RECEIVED_EMEANS"] : null ?>">
		</td>
		...
		<td colspan="4">
			<?
				...
				if(!empty($guue["AWARD_CONTRACT"]["ITEM_".$item]["AWARDED_CONTRACT"]["CONTRACTOR"])) {
					...
					if(!$ajax) { include 'forms/2_0_9_S3/common/ADDR-S5-f06.php'; } else { include '../common/ADDR-S5-f06.php'; }
					$contractor++;
				} else {
					$keys = '[AWARD_CONTRACT][ITEM_'.$item.'][AWARDED_CONTRACT][CONTRACTOR][ITEM_'.$contractor.'][ADDRESS_CONTRACTOR]';
					if(!$ajax) { include 'forms/2_0_9_S3/common/ADDR-S5-f06.php'; } else { include '../common/ADDR-S5-f06.php'; }
			...
			<table class="bordered valida" title="Valore totale inizialmente stimato del contratto d&#39;appalto/lotto" rel="<?= isRequired("total_final_value_of_the_contract") ?>;0;0;checked;group_validate">
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="text" title="Offerta pi&ugrave; bassa" value="<?= $valore_val_range_total_low ?>" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_RANGE_TOTAL][LOW]" <?= !$val_range_total ? 'disabled="disabled"' : null ?> id="item_<?= $item ?>_valore_val_range_total_low" rel="S;1;0;N">
						</td>
						<td>
							<input type="text" title="Offerta pi&ugrave; bassa" value="<?= $valore_val_range_total_high ?>" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_RANGE_TOTAL][HIGH]" <?= !$val_range_total ? 'disabled="disabled"' : null ?> id="item_<?= $item ?>_valore_val_range_total_high" rel="S;1;0;N">
						</td>
					</tr>
					<tr>
						<td class="etichetta" colspan="4">
	...
	<tr>
		...
		<td colspan="2">
			...
			<input type="text" title="Valore o percentuale del contratto d&#39;appalto da subappaltare a terzi" value="<?= $val_subcontracting ?>" name="guue[AWARD_CONTRACT][ITEM_<?= $item ?>][AWARDED_CONTRACT][VAL_SUBCONTRACTING][val]" <?= !$fields_subcontracted ? 'disabled="disabled"' : null ?> id="item_<?= $item ?>_subcontracting" rel="S;1;0;N">
		</td>
	</tr>
	<tr>
