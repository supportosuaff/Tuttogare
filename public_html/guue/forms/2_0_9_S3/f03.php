...
?>
<h2 style="text-align: right;"><b>Avviso di aggiudicazione di appalto</b></h2>
<p style="text-align: right; margin-top: 0px">Risultati della procedura di appalto<br>Direttiva 2014/24/UE</p>
<input type="hidden" name="tipologia_form" value="F03_2014">
<input type="hidden" name="numero_form" value="3">
...
<input type="hidden" name="attributi_form[FORM]" value="F03">
...
<?
	...
	$_SESSION["guue"]["numero_del_form"] = "f03";
		...
		1 => array(
			...
			1 => array(
				...
				1 => array(
					...
					6 => "sezioni/sezione_2_1_6_f03.php",
					8 => '</tbody></table>',
					),
				2 => array(
		...
		2 => array(
			...
			1 => array(
				0 => array(
					...
					1 => '<table class="bordered"><tbody>',
					2 => "sezioni/sezione_4_1_1_f03.php",
        ...
        foreach ($guue["AWARD_CONTRACT"] as $key_lot => $lot) {
          ...
          $j++;
			...
			if(! empty($guue["PROCEDURE"]["radio_as_select_for_procedure_type"])) {
				if($guue["PROCEDURE"]["radio_as_select_for_procedure_type"] == "PT_AWARD_CONTRACT_WITHOUT_CALL") {
					$f03[5][1] = "sezioni/annex_d1_f03.php";
				}
			}
        ...
        foreach ($guue["OBJECT_CONTRACT"]["OBJECT_DESCR"] as $key_lot => $lot) {
          if(strpos($key_lot,"ITEM_") !== FALSE) {
            $f03[1][1][2][2][] = array(
              ...
              5 => "sezioni/sezione_2_2_5_f03.php",
