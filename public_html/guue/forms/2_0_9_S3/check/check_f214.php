<?
	...
	if(!empty($_POST["guue"]["AWARD_CONTRACT"])) {
		$reset_no_awarded_to_group = array();
		$reset_no_sme = array();
		foreach ($_POST["guue"]["AWARD_CONTRACT"] as $award_contract_key => $award_contract) {
			if(!empty($award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"]) && $award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"] == "AWARDED_TO_GROUP" && (!is_array($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"]) || count($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"]) < 2)) {
				?>
					jalert('<div style="text-align:center">Impossibile salvare questo modello <strong>cod. #101</strong><br><strong> Nel caso di aggiudicazione a un raggruppamento di operatori economici sez. V.2) &egrave; necessario specificare tutti gli operatori nella sezione V.2.3) </strong></div>');
				<?
				die();
			}
			if(!empty($award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"]) && $award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"] == "NO_AWARDED_TO_GROUP" && (!is_array($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"]) || count($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"]) > 1)) {
				?>
					jalert('<div style="text-align:center">Impossibile salvare questo modello <strong>cod. #102</strong><br><strong> Nel caso di aggiudicazione a un solo operatore economico sez. V.2) &egrave; possibile indicare solo un operatore nella sezione V.2.3) </strong></div>');
				<?
				die();
			}
			$reset_no_awarded_to_group[$award_contract_key] = FALSE;
			if(!empty($award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"]) && $award_contract["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"] == "NO_AWARDED_TO_GROUP") {
				$reset_no_awarded_to_group[$award_contract_key] = TRUE;
				unset($_POST["guue"]["AWARD_CONTRACT"][$award_contract_key]["AWARDED_CONTRACT"]["radio_as_select_for_awarded_to_group"]);
			}

			$reset_no_sme[$award_contract_key] = array();
			if(!empty($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"])) {
				foreach ($award_contract["AWARDED_CONTRACT"]["CONTRACTOR"] as $contractor_key => $contractor_value) {
					$reset_no_sme[$award_contract_key][$contractor_key] = FALSE;
					if(isset($contractor_value["radio_as_select_for_is_an_sme"]) && $contractor_value["radio_as_select_for_is_an_sme"] == "NO_SME") {
						unset($_POST["guue"]["AWARD_CONTRACT"][$award_contract_key]["AWARDED_CONTRACT"]["CONTRACTOR"][$contractor_key]["radio_as_select_for_is_an_sme"]);
						$reset_no_sme[$award_contract_key][$contractor_key] = TRUE;
					}
				}
			}
		}
			...
			if(count($_POST["guue"]["OBJECT_CONTRACT"]["OBJECT_DESCR"]) < 2) {
				...
				if(!empty($_POST["guue"]["OBJECT_CONTRACT"]["OBJECT_DESCR"])) {
					foreach ($_POST["guue"]["OBJECT_CONTRACT"]["OBJECT_DESCR"] as $lot_key => $lot) {
						if(empty($lot["TITLE"])) {
							?>
							jalert('<div style="text-align:center">Impossibile salvare questo modello <strong>cod. #224</strong><br><strong>Nelle informazioni relative ai lotti II.1.6) è stato indicata la presenza di più lotti perciò nella sezione II.2.1) è necessario specificare una denominazione per ciascun lotto</strong></div>');
							<?
							die();
						}
					}
