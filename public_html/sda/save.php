	...
	if (!$edit) {
		...
		if (isset($_POST["operazione"])) {
			...
			if ($_POST["operazione"]=="INSERT") {
				...
				$sql = "SELECT max(id) as id FROM b_bandi_sda WHERE codice_gestore = :codice_ente GROUP BY codice_gestore ";
			...
			$salva->nome_tabella = "b_bandi_sda";
			...
			if (isset($_POST["cpv"])) {
						if ($_POST["cpv"] != "")  {
							...
							$strsql = "DELETE FROM r_cpv_bandi_sda WHERE codice_bando = :codice_bando ";
							...
							foreach($array_cpv as $cpv) {
								if ($cpv != "") {
									...
									$salva->debug = false;
									$salva->codop = $_SESSION["codice_utente"];
									$salva->nome_tabella = "r_cpv_bandi_sda";
			...
			$href = "/sda/pannello.php?codice=" . $codice_bando;
