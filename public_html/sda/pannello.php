<?
	...
	if (isset($_SESSION["codice_utente"]) && isset($_SESSION["ente"])) {
		...
		if (isset($_GET["codice"])) {
				$codice = $_GET["codice"];
				$bind = array(":codice"=>$codice,":codice_gestore"=>$_SESSION["ente"]["codice"]);
				$strsql  = "SELECT * ";
				$strsql .= "FROM b_bandi_sda ";
				$strsql .= "WHERE codice = :codice ";
				$strsql .= "AND codice_gestore = :codice_gestore ";
				...
				if ($risultato->rowCount() > 0) {
                    ...
                    <a class="pannello" href="partecipanti/index.php?codice=<? echo $record["codice"]?>" title="Partecipanti">Partecipanti
						<?
							...
							$strsql = "SELECT * FROM r_partecipanti_sda WHERE codice_bando = :codice AND valutato = 'N' AND conferma ='S' ";
