...
if (isset($_GET["codice"])) {
	...
	if ($risultato->rowCount()>0) {
		...
		if (!empty($ris_form)) {
			...
			$percentuale = "33";
			$sql_check = "SELECT * FROM r_dgue_gare WHERE codice_gara = :codice_gara AND sezione = 'sda'";
			$ris_check = $pdo->bindAndExec($sql_check,$bind);
			if ($ris_check->rowCount() > 0) {
				?>
				<div style="float:left; width:<?= $percentuale ?>%">
					<button onClick="elimina('<? echo $record["codice"] ?>','sda/dgue');" class="submit_big" style="background-color:#c00"><h4 style="text-align:center"><span class="fa fa-remove fa-2x"></span> Elimina DGUE</h4></button>
				</div>
				<div style="float:left; width:<?= $percentuale ?>%">
					<a href="/dgue/getRequestPDF.php?codice_riferimento=<?= $record["codice"] ?>&sezione=sda" class="submit_big" style="background-color:#900"><h4 style="text-align:center"><span class="fa fa-file-pdf-o fa-2x"></span> Download PDF</h4></a>
				</div>
				<div style="float:left; width:<?= $percentuale ?>%">
					<a href="/dgue/getRequestXML.php?codice_riferimento=<?= $record["codice"] ?>&sezione=sda" class="submit_big" style="background-color:#066"><h4 style="text-align:center"><span class="fa fa-code fa-2x"></span> Download XML</h4></a>
				</div>
			...
			<form name="box" method="post" action="save.php" rel="validate">
				...
				foreach($ris_form AS $form) {
					if ($form["livello2"] != "" && $form["obbligatorio"] == "N") {
						...
						$sql_selezionato = "SELECT * FROM r_dgue_gare WHERE codice_gara = :codice_gara AND sezione = 'sda' AND codice_form = :codice_form";
