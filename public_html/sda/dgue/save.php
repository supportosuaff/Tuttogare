<?
	...
	if ($edit) {
			...
			$sql = "DELETE FROM r_dgue_gare WHERE codice_gara = :codice_gara AND sezione = 'sda' ";
			...
			if (isset($_POST["form"])) {
				...
				$sql = "SELECT * FROM b_modulistica_sda WHERE titolo = 'DGUE' AND codice_bando = :codice_gara ";
				$bind = array(":codice_gara"=>$codice_gara);
				...
				if ($ris->rowCount() == 0) {
					...
					$salva->debug = false;
					$salva->codop = $_SESSION["codice_utente"];
					$salva->nome_tabella = "b_modulistica_sda";
					$salva->operazione = "INSERT";
					$salva->oggetto = $object;
					$salva->save();
				}
			...
			$href = "/sda/dgue/edit.php?codice=" . $codice_gara;
