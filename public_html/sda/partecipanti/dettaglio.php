	...
	if (isset($_SESSION["codice_utente"]) && isset($_SESSION["ente"])) {
		...
		if (isset($_GET["codice"])&&isset($_GET["codice_bando"])) {
					...
					if ($risultato->rowCount() > 0) {
						...
						$strsql = "SELECT b_cpv.* FROM b_cpv JOIN r_cpv_bandi_sda ON b_cpv.codice = r_cpv_bandi_sda.codice WHERE r_cpv_bandi_sda.codice_bando = :codice ORDER BY codice";
						...
						$strsql = "UPDATE r_partecipanti_sda SET visto = 'S' WHERE codice_bando = :codice_bando AND codice_operatore = :codice_operatore ";
						$risultato = $pdo->bindAndExec($strsql,$bind);
	  ...
	  <td class="etichetta">Data emissione</td><td><? echo mysql2date($record_operatore["data_emissione_certificato"]) ?></td></tr>
	                     ...
	                     <h1>Categorie merceologiche</h1>
	                        <?
													...
													$strsql  = "SELECT b_cpv.* FROM b_cpv JOIN r_cpv_operatori_sda ON b_cpv.codice = r_cpv_operatori_sda.codice WHERE codice_operatore = :codice_operatore AND codice_bando = :codice_bando ";
						 ...
						 ?>
	                      ...
	                      <?
	                        ...
	                        $sql = "SELECT * FROM b_modulistica_sda WHERE codice_bando = :codice_bando ORDER BY attivo DESC, codice";
	                            ...
	                            if($ris_allegato->rowCount()>0) {
	                              ...
	                              $nome_file.= "<div style=\"display:none;\" id=\"note_" . $allegato["codice"] . "\">" . $note . "</div>";
													 ...
													 <div id="comunicazioni">
 														<?
 															...
 															r_comunicazioni_utenti.codice_comunicazione
 																				 WHERE  b_comunicazioni.sezione = 'sda' AND b_comunicazioni.codice_gara = :codice_bando AND
									 ...
									 <? if ($record_partecipante["ammesso"] != 'S' || $record_partecipante["valutato"] == 'N') { ?>
													<form action="/sda/abilitazione/abilita.php" method="post">
	                              ...
	                              <input type="submit" class="submit_big" style="background-color:#0C0" value="Abilita">
										...
										<? if (($record_partecipante["ammesso"] != 'N' && $record_partecipante["valutato"] == "S") || $record_partecipante["valutato"] == "N") { ?>
														...
														<div id="form_dialog" style="display:none">
																<form action="/sda/abilitazione/respingi.php" rel="validate" method="post">
																			<input type="hidden" name="codice_operatore" value="<?= $codice ?>">
																			<input type="hidden" name="codice_bando" value="<?= $record_bando["codice"] ?>">
										...
										<? } ?>
														<script>
															function ritorna() {
																window.location.href = "/sda/partecipanti/index.php?codice=<? echo $record_bando["codice"] ?>";
															}
