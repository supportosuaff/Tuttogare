	...
	$edit = false;
			...
			die();
				...
				if ($risultato->rowCount() > 0) {
					...
					<?
					$schede = array();
					$schede["nuove_istanze"] = "r_partecipanti_sda.valutato = 'N' AND (r_partecipanti_sda.timestamp_abilitazione IS NULL)";
					$schede["aggiornamenti"] = "r_partecipanti_sda.valutato = 'N' AND (r_partecipanti_sda.timestamp_abilitazione IS NOT NULL)";
					$schede["ammessi"] = " r_partecipanti_sda.ammesso = 'S' AND r_partecipanti_sda.valutato = 'S' ";
					$schede["respinti"] = " r_partecipanti_sda.ammesso = 'N' AND r_partecipanti_sda.valutato = 'S' ";
					$ris_array = array();
					$bind = array();
					$bind[":codice"] = $record["codice"];
					foreach($schede AS $key => $where_statement) {
						$sql = "SELECT b_operatori_economici.*, r_partecipanti_sda.visto,
										r_partecipanti_sda.timestamp_richiesta AS ricezione,
										r_partecipanti_sda.timestamp_aggiornamento AS aggiornamento,
										r_partecipanti_sda.timestamp_abilitazione AS abilitazione,
										r_partecipanti_sda.timestamp AS ultima_modifica
										FROM b_operatori_economici
										JOIN r_partecipanti_sda ON b_operatori_economici.codice = r_partecipanti_sda.codice_operatore
										WHERE {$where_statement}
										AND r_partecipanti_sda.codice_bando = :codice
										AND r_partecipanti_sda.conferma = 'S'
										GROUP BY b_operatori_economici.codice
										ORDER BY r_partecipanti_sda.timestamp_richiesta ASC, r_partecipanti_sda.visto";
					...
					?>
						...
						foreach($ris_array AS $key => $ris_partecipanti) {
							...
							<table width="100%" class="elenco">
								...
								<?
									$_SESSION["exportElencoPartecipanti"]["sda"][$codice][$key] = $export;
								?>
							</div>
  						<?
  							}
