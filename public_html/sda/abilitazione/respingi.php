...
if (!$edit) {
	...
	if ($risultato->rowCount() > 0) {
		...
		$strsql= "SELECT b_bandi_sda.* FROM b_bandi_sda JOIN r_partecipanti_sda ON b_bandi_sda.codice = r_partecipanti_sda.codice_bando WHERE
							b_bandi_sda.codice = :codice_bando
							AND r_partecipanti_sda.codice_operatore = :codice_operatore ";
		$risultato = $pdo->bindAndExec($strsql,$bind);
		if ($risultato->rowCount() > 0) {
			$record_bando = $risultato->fetch(PDO::FETCH_ASSOC);
			$strsql = "UPDATE r_partecipanti_sda SET valutato = 'S', ammesso = 'N' WHERE codice_operatore = :codice_operatore AND codice_bando = :codice_bando";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			...
			$corpo = "Si informa che a seguito di valutazione, la S.V. non &egrave; stata abilitata al sistema dinamico di acquisizione relativo al bando:<br>";
