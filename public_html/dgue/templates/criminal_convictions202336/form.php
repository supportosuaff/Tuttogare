<?
  ...
  if (isset($_POST["id"]) && isset($_POST["target"])) {
    ...
    $sql_form = "SELECT * FROM b_dgue_settings WHERE uuid = :uuid  AND version = '2023-36'";
    ...
    for ($count_element = 0;$count_element < $to;$count_element++) {
        ...
        <div id="criteria_<?= $form["codice"] ?>_<?= $id_repeat ?>" <? if ($values["_0"][0] != "true") echo "style='display:none'" ?>>
            <table width="100%">
              ...
              <tr>
                ...
                <td>
                  <div style="text-align:center">
                    <input type="hidden" name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:Requirement][5][cbc:ID][$]" value="PERS_ID_PENAACCESSORIA-CONTR-PA-INDICATOR">
                    <br>
                    <label>Si</label>
                    <input data-show="#criteria_<?= $form["codice"] ?>_<?= $id_repeat ?>_pena_accessoria" type="radio"
                      name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:Requirement][5][ccv:Response][ccv-cbc:Indicator]"
                      value="true" <? if ($values["_0"][5] == "true") echo "checked='checked'" ?>
                      onclick="show_hide($(this));">
                    <label>No</label>
                    <input data-hide="#criteria_<?= $form["codice"] ?>_<?= $id_repeat ?>_pena_accessoria" type="radio"
                      name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:Requirement][5][ccv:Response][ccv-cbc:Indicator]"
                      value="false" <? if ($values["_0"][5] == "false") echo "checked='checked'" ?>
                      onclick="show_hide($(this));">
                  </div>
                </td>
              </tr>
              <tr id="criteria_<?= $form["codice"] ?>_<?= $id_repeat ?>_pena_accessoria" <? if ($values["_0"][5] != "true") echo "style='display:none'" ?>>
                <td class="etichetta">
                  Durata della pena accessoria
                </td>
                <td>
                  <input type="hidden" name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:Requirement][6][cbc:ID][$]" value="PERS_ID_PENAACCESSORIA-CONTR-PA-PERIOD">
                  <input type="text" rel="N;1;0;A" title="Durata della pena accessoria" class="dgue_input"
                  name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:Requirement][6][ccv:Response][cac:Period][cbc:Description]"
                  value="<?= $values["_0"][6] ?>">
                </td>
              </tr>
            </table>
              ...
              <tr class="autodisciplina_<?= $form["codice"] ?>_<?= $id_repeat ?>" <? if ($values["_01"][0] != "true") echo "style='display:none'" ?>>
                <td>
                  <table width="100%">
                    ...
                    <tr>
                      <td class="etichetta">
                        2) Si &egrave; impegnato formalmente a risarcire il danno?
                      </td>
                    </tr>
                    <tr>
                      <td style="text-align:center">
                    ...
                    <tr>
                      <td style="text-align:center">
                        <label>Si</label>
                        <input class="form_autodisciplina_<?= $form["codice"] ?>_<?= $id_repeat ?>" type="hidden" name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:RequirementGroup][ccv:Requirement][9][cbc:ID][$]" value="PERS_ID_SELFCLEAN_9">
                        <input type="radio"
                          name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:RequirementGroup][ccv:Requirement][9][ccv:Response][ccv-cbc:Indicator]"
                          value="true"
                          class="form_autodisciplina_<?= $form["codice"] ?>_<?= $id_repeat ?>"
                          <? if ($values["_01"][9] == "true") echo "checked='checked'" ?>>
                        <label>No</label>
                        <input type="radio"
                          name="espd[ccv:Criterion][<?= $form["uuid"] ?>][<?= $id_repeat ?>][ccv:RequirementGroup][0][ccv:RequirementGroup][ccv:Requirement][9][ccv:Response][ccv-cbc:Indicator]"
                          value="false"
                          class="form_autodisciplina_<?= $form["codice"] ?>_<?= $id_repeat ?>"
                            <? if ($values["_01"][9] == "false") echo "checked='checked'" ?>>
                      </td>
                    </tr>
                    <tr>
                      <td class="etichetta">
      ...
      <script>
        $(".form_autodisciplina_<?= $form["codice"] ?>_<?= $id_repeat ?>").change(function(){
          ...
          $.ajax({
            url: '/dgue/templates/criminal_convictions202336/elabora.php',
            type: 'POST',
            data: data,
            dataType: 'html'
