  ...
  if (isset($form)) {
    ...
    <div id="criteria_<?= $form["codice"] ?>" <? if ($values["_0"][0] != "false") echo "style='display:none'" ?>>
        <table width="100%">
          <tr>
            <td class="etichetta">
              In caso negativo, spiegare perch&eacute; e precisare di quali altri mezzi di prova relativi ai sistemi o alle norme di gestione ambientale si dispone:
            </td>
            <td>
              <input type="hidden" name="espd[ccv:Criterion][<?= $form["uuid"] ?>][ccv:RequirementGroup][0][ccv:Requirement][1][cbc:ID][$]" value="b0aace10-fd73-46d1-ae78-289ee5cd42ca">
