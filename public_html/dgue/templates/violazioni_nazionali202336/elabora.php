...
;
  ...
  if (is_operatore() && isset($_POST["espd"])) {
    ...
    $json = json_decode(file_get_contents($root."/dgue/templates/integrazioni_202336/definition.json"),true);
    foreach($_POST["espd"]["ccv:Criterion"] AS $uuid => $singleCriteria) {
      ...
      if (!empty($values["_0"][0])) {
        ...
        $sql = "SELECT * FROM b_dgue_settings WHERE uuid = :uuid AND version = '2023-36' AND attivo = 'S'";
