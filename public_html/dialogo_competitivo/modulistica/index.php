            ...
            <input type="hidden" name="codice_bando" value="<? echo $record["codice"]; ?>">
					...
					<table id="tab_moduli" width="100%">
						...
						<?
							...
							$strsql = "SELECT * FROM b_modulistica_dialogo WHERE codice_bando = :codice AND attivo = 'S'";
							$risultato = $pdo->bindAndExec($strsql,$bind);
							if ($risultato->rowCount()>0) {
								while($modulo=$risultato->fetch(PDO::FETCH_ASSOC)) {
									$id = $modulo["codice"];
									if ($modulo["nome_file"] != "") $modulo["nome_file"] = "<a href=\"/documenti/allegati/dialogo/" . $modulo["codice_bando"] . "/" . $modulo["nome_file"] . "\" target=\"_blank\">" . $modulo["nome_file"] . "</a>";
