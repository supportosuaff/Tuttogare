	...
	if (!$edit) {
		...
		if ($risultato->rowCount() > 0) {
			...
			if ($risultato->rowCount() > 0) {
				...
				$strsql = "UPDATE r_partecipanti_dialogo SET valutato = 'S', ammesso = 'S', timestamp_abilitazione = :timestamp WHERE codice_operatore = :codice_operatore AND codice_bando = :codice_bando";
				$risultato = $pdo->bindAndExec($strsql,$bind);
				...
				$corpo = "Si informa che a seguito di valutazione, la S.V. &egrave; stata abilitata al dialogo competitivo relativo al bando:<br>";
