		...
		if (isset($_GET["cod"]) && isset($_SESSION["ente"]["codice"])) {
				...
				if (!isset($_SESSION["codice_utente"])) {
					...
					$strsql .= "AND pubblica = '2' ";
						...
						$strsql  = "SELECT b_bandi_dialogo.* FROM b_bandi_dialogo WHERE b_bandi_dialogo.codice = :codice ";
						$strsql .= "AND (codice_ente = :codice_ente OR codice_gestore = :codice_ente) ";
				...
				if ($risultato->rowCount() > 0) {
					...
					if ($_SESSION["language"] != "IT") {
						$strsql = "SELECT b_cpv_dict.*, b_cpv_dict.{$_SESSION["language"]} AS descrizione FROM b_cpv_dict JOIN r_cpv_bandi_dialogo ON b_cpv_dict.codice = r_cpv_bandi_dialogo.codice WHERE r_cpv_bandi_dialogo.codice_bando = :codice ORDER BY b_cpv_dict.codice";
					} else {
						$strsql = "SELECT b_cpv.* FROM b_cpv JOIN r_cpv_bandi_dialogo ON b_cpv.codice = r_cpv_bandi_dialogo.codice WHERE r_cpv_bandi_dialogo.codice_bando = :codice ORDER BY b_cpv.codice";
					}
					...
					$sql = "SELECT * FROM b_allegati WHERE codice_gara = :codice AND sezione = 'dialogo' AND online = 'S'";
...
?>
<h1><?= traduci("DETTAGLI") ?> - ID <? echo $record["id"] ?></h1>
  <? if ($record["annullata"] == "S") {
	...
	if (($record["annullata"] == "N") && ((strtotime($record["data_scadenza"]) > time())||($record["data_scadenza"] == 0))) {
		?><div class="box"><?
		$sql = "SELECT * FROM r_dgue_gare WHERE codice_gara = :codice_gara AND sezione = 'dialogo' ";
		...
		if ($ris_dgue->rowCount() > 0) {
			...
			if (strtotime($record["data_scadenza"]) > time() || $record["data_scadenza"]==0) {
				if (is_operatore()) {
					?>
					<a href="/dgue/edit.php?sezione=dialogo&codice_riferimento=<?= $record["codice"] ?>" style="background-color:#055" class="submit_big"><?= traduci("Compila Documento di Gara Unico Europeo") ?> - <?= traduci("DGUE") ?></a>
					<?
		...
		if (is_operatore()) {
			...
			$sql = "SELECT * FROM r_partecipanti_dialogo WHERE codice_bando = :codice AND codice_utente = :codice_utente ";
		...
		?>
        	<a href="/dialogo_competitivo/modulo.php?cod=<? echo $record["codice"] ?>" class="submit_big" title="<?= $testo ?>"><?= $testo ?></a>
        <?
        	if ($partecipato) {
				$par = $ris->fetch(PDO::FETCH_ASSOC);
				?>
				<a id="button_revoca_<? echo $par["codice"] ?>" href="#" onClick="elimina('<? echo $par["codice"] ?>','dialogo_competitivo/abilitazione');" class="submit_big" style="background-color:#C30" title="<?= traduci("Revoca partecipazione") ?>"><?= traduci("Revoca partecipazione") ?></a>
                <? }
		} else if (!isset($_SESSION["codice_utente"])) {
			if ($ris_dgue->rowCount() == 0) {
			?>
	...
	<td class="etichetta"><?= traduci("Responsabile del procedimento") ?></td><td><? echo $record["rup"] ?></td></tr>
                        <? if ((isset($ris_allegati) && ($ris_allegati->rowCount() > 0)) || (!empty($dgue) && $dgue)) { ?>
                            ...
                            <table width="100%" id="tab_allegati">
													        ...
													        <td>
																		<strong><a href="/dgue/getRequestXML.php?sezione=dialogo&codice_riferimento=<?= $record["codice"] ?>">XML <?= traduci("Richiesta DGUE") ?></a></strong>
																	</td>
													      </tr>
															<? } ?>
                            <?
...
<?
			...
			$bind = array(':codice' => $record["codice"]);
			$sql = "SELECT * FROM b_modulistica_dialogo WHERE codice_bando = :codice AND attivo = 'S' ORDER BY codice";
			$risultato = $pdo->bindAndExec($sql,$bind);
			if ($risultato->rowCount()>0 || $dgue) { ?>
				<div class="box">
						...
						<tr>
							<td>
								...
								if ($record_modulo["nome_file"]!= "") {
									?>
									<br>
									<a href="/documenti/allegati/dialogo/<?= $record["codice"] ?>/<?= $record_modulo["nome_file"] ?>" target="_blank"><?=traduci("Scarica") ?></a>
									<?
								}
								?>
							</td>
