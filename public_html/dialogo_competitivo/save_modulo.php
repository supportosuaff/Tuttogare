		...
		if (is_operatore()) {
			if (isset($_POST["codice_bando"])) {
				...
				if ($risultato->rowCount() > 0) {
						...
						if ($risultato->rowCount()>0) {
							$errore_upload = false;
							include($root."/inc/aggiorna_pem.php");
								...
								if (isset($_POST["filechunk_".$record_modulo["codice"]]) && ($_POST["filechunk_".$record_modulo["codice"]] != "") && (strpos($_POST["filechunk_".$record_modulo["codice"]], "../")===false)) {
									...
									if ($md5_file == $_POST["md5_file_".$record_modulo["codice"]]) {
										...
										if ($esito == "Verification successful") {
											...
											$msg .= "</ul>";
											$msg .= "</li>";
											if (!is_dir($config["arch_folder"] . "/allegati_dialogo/" . $operatore["codice"] . "/")) mkdir($config["arch_folder"] . "/allegati_dialogo/" . $operatore["codice"],0770,true);
											$allegato = array();
											$allegato["codice_modulo"] = $record_modulo["codice"];
											$allegato["codice_operatore"] = $operatore["codice"];
											$path_file = $config["arch_folder"] ."/allegati_dialogo/" . $operatore["codice"];
											...
											if ($copy != false) {
												...
												$sql = "SELECT * FROM b_allegati_dialogo WHERE ";
												...
												$salva->nome_tabella = "b_allegati_dialogo";
						...
						$bind = array();
						$bind[":codice"] = $record_bando["codice"];
						$sql = "SELECT * FROM b_modulistica_dialogo WHERE codice_bando = :codice AND obbligatorio = 'S' AND attivo = 'S' ORDER BY codice";
						$risultato = $pdo->bindAndExec($sql,$bind);
						if ($risultato->rowCount()>0) {
							while ($record_modulo = $risultato->fetch(PDO::FETCH_ASSOC)) {
								$bind = array();
						...
						if (isset($_POST["invia"]) && $_POST["invia"]=="S") {
							if (!$errore_allegati) {
								...
								$salva->debug = false;
								$salva->codop = $_SESSION["codice_utente"];
								$salva->nome_tabella = "r_partecipanti_dialogo";
								...
								if ($codice_partecipante!=false) {
									...
									$corpo = "L'operatore economico " . $operatore["codice_fiscale_impresa"] . " " . $operatore["ragione_sociale"] . ",  ha richiesto l'abilitazione al dialogo competitivo:<br>";
