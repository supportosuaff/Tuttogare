	...
	if (isset($_SESSION["codice_utente"]) && isset($_SESSION["ente"])) {
		...
		if (isset($_GET["codice"])) {
				...
				if ($risultato->rowCount() > 0) {
					...
					?>
					<h1>PANNELLO DI GESTIONE - BANDO #<? echo $record["id"] ?> <input type="image" onclick="$('#info').toggle()" src="/img/info.png" title="Ulteriori informazioni"></h1>
                    ...
                    <? if (strtotime($record["data_scadenza"]) <= time()) { ?><a class="pannello" href="partecipanti/index.php?codice=<? echo $record["codice"]?>" title="Partecipanti">Partecipanti
						<?
							$bind = array();
							$bind[":codice_bando"] = $record["codice"];
							$strsql = "SELECT * FROM r_partecipanti_dialogo WHERE codice_bando = :codice_bando AND valutato = 'N' AND conferma = 'S'";
