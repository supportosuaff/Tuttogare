	...
	$public = true;
		if ((isset($_GET["cod"]) || isset($_POST["cod"]))&& is_operatore()) {
				...
				if ($risultato->rowCount() > 0) {
					...
					?>
					<h1><?= traduci("PRESENTAZIONE ISTANZA") ?> - ID <? echo $record_bando["id"] ?></h1>
					...
					<?
						if (is_operatore()) {
								...
								$sql = "SELECT * FROM r_partecipanti_dialogo WHERE codice_bando = :codice_bando AND codice_utente = :codice_utente";
								...
								<form id="modulo" rel="validate" method="post" target="_self" action="save_modulo.php">
									...
									<? } ?>
										<table width="100%">
											...
											<?
												$bind = array();
												$bind[":codice_bando"] = $record_bando["codice"];
												$sql = "SELECT * FROM b_modulistica_dialogo WHERE codice_bando = :codice_bando AND attivo = 'S' ORDER BY codice";
									...
									<?
										if (!$partecipato) {
											...
											?>
											<a id="button_revoca_<? echo $partecipante["codice"] ?>" href="#" onClick="elimina('<? echo $partecipante["codice"] ?>','dialogo_competitivo/abilitazione');" class="submit_big" style="background-color:#C30" title="<?= traduci("Revoca partecipazione") ?>"><?= traduci("Revoca partecipazione") ?></a>
											<?
										}
									?>
								</form>
