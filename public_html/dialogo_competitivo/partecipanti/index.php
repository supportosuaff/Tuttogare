	...
	$edit = false;
			...
			die();
				...
				if ($risultato->rowCount() > 0) {
					...
					<?
					$schede = array();
					$schede["nuove_istanze"] = "r_partecipanti_dialogo.valutato = 'N' AND (r_partecipanti_dialogo.timestamp_abilitazione IS NULL)";
					$schede["aggiornamenti"] = "r_partecipanti_dialogo.valutato = 'N' AND (r_partecipanti_dialogo.timestamp_abilitazione IS NOT NULL)";
					$schede["ammessi"] = " r_partecipanti_dialogo.ammesso = 'S' AND r_partecipanti_dialogo.valutato = 'S' ";
					$schede["respinti"] = " r_partecipanti_dialogo.ammesso = 'N' AND r_partecipanti_dialogo.valutato = 'S' ";
					$ris_array = array();
					$bind = array();
					$bind[":codice"] = $record["codice"];
					foreach($schede AS $key => $where_statement) {
						$sql = "SELECT b_operatori_economici.*, r_partecipanti_dialogo.visto,
										r_partecipanti_dialogo.timestamp_richiesta AS ricezione,
										r_partecipanti_dialogo.timestamp_aggiornamento AS aggiornamento,
										r_partecipanti_dialogo.timestamp_abilitazione AS abilitazione,
										r_partecipanti_dialogo.timestamp AS ultima_modifica
										FROM b_operatori_economici
										JOIN r_partecipanti_dialogo ON b_operatori_economici.codice = r_partecipanti_dialogo.codice_operatore
										WHERE {$where_statement}
										AND r_partecipanti_dialogo.codice_bando = :codice
										AND r_partecipanti_dialogo.conferma = 'S'
										GROUP BY b_operatori_economici.codice
										ORDER BY r_partecipanti_dialogo.timestamp_richiesta ASC, r_partecipanti_dialogo.visto";
					...
					?>
						...
						foreach($ris_array AS $key => $ris_partecipanti) {
							...
							<table width="100%" class="elenco">
								...
								<?
									$_SESSION["exportElencoPartecipanti"]["dialogo"][$codice][$key] = $export;
								?>
							</div>
  						<?
  							}
