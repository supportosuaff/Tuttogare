...
?><h1>Concorsi Attivi</h1><?
	if ($risultato->rowCount() > 0) {
	?>

    <table width="100%" id="concorsi" class="elenco">
    	<thead>
				<tr><td></td><td>ID</td><td>Stato</td><td>CIG</td><td>Oggetto</td>
    ...
    <?
		while ($record = $risultato->fetch(PDO::FETCH_ASSOC)) {
			$show = false;
			if (($record["stato"]==3) && (strtotime($record["data_scadenza"])>time()) && isset($_SESSION["codice_utente"])) {
    ...
    <tr id="<? echo $record["codice"] ?>">
      ...
      <td width="10%"><? echo $record["fase"] ?></td>
      ...
      <td width="15%"><? echo mysql2datetime($record["data_scadenza"]) ?></td>
     </tr>
        <?
			}
		}
