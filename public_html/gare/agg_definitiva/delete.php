	...
	if (!$edit) {
		...
		if (isset($_POST["codice"])) {
			...
			if ($risultato->rowCount()>0) {
				...
				log_gare($_SESSION["ente"]["codice"],$record["codice_gara"],"DELETE","Modello comunicazione esito");
				?>
	         alert("Rielaborazione effettuata con successo");
	         window.location.href = '/gare/agg_definitiva/avvisi_esito.php?codice=<? echo $record["codice_gara"] ?>';
			   <?
			 } else {
				 ?>
