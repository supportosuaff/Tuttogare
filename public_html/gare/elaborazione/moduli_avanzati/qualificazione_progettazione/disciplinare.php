...
if ($ris_classificazioni->rowCount()>0) {
	$html .="<table style=\"width:100%\"><thead><tr><th style=\"width:5%\">ID</th><th style=\"width:5%\">Classe e Categoria</th><th style=\"width:10%\">Grado di complessit&agrave</th><th style=\"width:70%\">Declaratoria sintetica</th><th style=\"width:10%\">Importo</th></tr></thead><tbody>";
	while ($rec_qualificazione = $ris_classificazioni->fetch(PDO::FETCH_ASSOC)) {
		$html.= "<tr><td>" . $rec_qualificazione["id"] . "</td><td>" . $rec_qualificazione["classe"] . "</td><td>" . number_format($rec_qualificazione["complessita"],2,",",".") . "</td><td>" . $rec_qualificazione["descrizione"] . "</td><td>&euro; ".number_format($rec_qualificazione["importo"],2,",",".")."</td></tr>";
	}
	$html.= "</tbody></table>";
}
?>
