<?
	...
	if ($edit && !$lock) {
		...
		if (isset($_POST["private_key"])) {
			...
			$strsql  = "SELECT b_rdo_ad.*, r_partecipanti.pec, r_partecipanti.partita_iva, r_partecipanti.ragione_sociale, r_rdo_ad.aperto,
									r_rdo_ad.codice_partecipante,r_rdo_ad.salt,r_rdo_ad.nome_file, b_gare.codice_pec, b_gare.oggetto, b_gare.public_key FROM
									...
									AND b_rdo_ad.data_apertura <= now() ";
			...
			if ($risultato->rowCount() > 0) {
				...
				if ($rdo["aperto"]=="N") {
							...
							$enc_data = file_get_contents($config["doc_folder"] . "/" . $rdo["codice_gara"] . "/rdo/".$rdo["codice"]."/" . $rdo["nome_file"]);
								if (@openssl_private_decrypt($rdo["salt"],$salt,$key)) {
									$data = openssl_decrypt($enc_data,$config["crypt_alg"],$salt,OPENSSL_RAW_DATA,$config["enc_salt"]);
									$oggetto = "Apertura offerta per la richiesta: " . $rdo["oggetto"];
									$corpo = "Si comunica l'avvenuta apertura dell'offerta trasmessa riferita alla richiesta:<br><br>";
									$corpo = "<strong>".$rdo["richiesta"]."</strong><br><br>";
									$corpo.= "Data e ora di apertura: <strong>" . date("d/m/Y H:i:s") . "</strong><br><br>";
									$corpo.= "Distinti Saluti<br><br>";

									$bind = array();
									$bind[":codice_gara"] = $_POST["codice_gara"];
									$bind[":codice_partecipante"] = $rdo["codice_partecipante"];
									...
									if (!is_dir($config["arch_folder"]."/" . $rdo["codice_gara"] . "/rdo/".$rdo["codice"]."/")) mkdir($config["arch_folder"]."/" . $rdo["codice_gara"] . "/rdo/".$rdo["codice"]."/",0777,true);
									$percorso = $config["arch_folder"]."/" . $rdo["codice_gara"] . "/rdo/".$rdo["codice"]."/";
									...
									$allegato["titolo"] = "Offerta " . $rdo["partita_iva"];
									$allegato["online"] = "N";
									...
									$sql = "UPDATE r_rdo_ad SET aperto = 'S', codice_allegato = :codice_allegato WHERE codice = :codice";
									$ris = $pdo->bindAndExec($sql,$bind);
									$note = "";
									$class= "";
									log_gare($_SESSION["ente"]["codice"],$_POST["codice_gara"],"APERTURA","Aperta offerta Codice: " . $codice);
									...
									?>
									$("#cella_<? echo $rdo["codice_partecipante"] ?>").html('<? echo $cella ?>');
