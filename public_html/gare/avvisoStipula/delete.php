<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"])) {
			...
			if (isset($_SESSION["ente"])) { $bind[":codice_ente"] = $_SESSION["ente"]["codice"]; $strsql.= " AND codice_ente = :codice_ente"; }
			$risultato = $pdo->bindAndExec($strsql,$bind);
			if ($risultato->rowCount()>0) {
				scrivilog("b_documentale","DELETE",$pdo->getSQL(),$_SESSION["codice_utente"]);
				
				log_gare($_SESSION["ente"]["codice"],$record["codice_gara"],"DELETE","Modello comunicazione stipula contratto");
				?>
	         alert("Rielaborazione effettuata con successo");
	         window.location.href = '/gare/avvisoStipula/index.php?codice=<? echo $record["codice_gara"] ?>';
			   <?
			 } else {
				 ?>
