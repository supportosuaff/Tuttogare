		...
		if ((isset($_GET["codice"]) || isset($_GET["cod"]))) {
				...
				if ($ris_lotti->rowCount()>0) {
					if (isset($_GET["lotto"])) {
						...
						?>
							<a class="submit_big" href ="verbale_ammissione.php?codice=<? echo $_GET["codice"] ?>&lotto=<? echo $lotto["codice"] ?>">
								<? echo $lotto["oggetto"]; ?>
							</a>
						<?
						}
				...
				if ($print_form) {
					...
					$editor_tipo = "verbale_ammissione";
					$bind = array();
					...
					if ($risultato->rowCount()>0) {
						...
						$ris_punteggi = $ris_punteggi->fetchAll(PDO::FETCH_ASSOC);
							...
							$sql = "SELECT b_log_aperture.timestamp, r_partecipanti.partita_iva, r_partecipanti.ragione_sociale
											...
											AND b_criteri_buste.tecnica = 'N' AND b_criteri_buste.economica = 'N'
											ORDER BY timestamp ";
							...
							$ris_soccorsi = $pdo->go($sql_soccorsi,[":codice_lotto"=>$codice_lotto,":codice_gara"=>$record_gara["codice"]]);
							...
							$sql_modello = "SELECT * FROM b_modelli_standard WHERE codice = 2"; // Codice 2 - I Verbale
				...
				?>
					<form name="box" method="post" action="salva_I.php" rel="validate">
