		...
		if ($risultato->rowCount() > 0) {
				...
				if ($ris_lotti->rowCount()>0) {
					if (isset($_GET["lotto"])) {
						...
						if ($ris_lotti->rowCount()>0) {
							$print_form = true;
							$risultato = $pdo->go("SELECT * FROM b_nso WHERE codice_gara = :codice_gara AND codice_lotto = :codice_lotto AND codice_gestore = :codice_gestore ",
					...
					$print_form = true;
					$codice_lotto = 0;
					$risultato = $pdo->go("SELECT * FROM b_nso WHERE codice_gara = :codice_gara AND codice_lotto IS NULL AND codice_gestore = :codice_gestore ",
					[":codice_gara"=>$codice,":codice_gestore"=>$_SESSION["ente"]["codice"]]);
				}
			if ($print_form) {
				$path="/nso/edit.php?codice=0&codice_gara={$codice}&codice_lotto={$codice_lotto}";
				if ($risultato->rowCount() > 0) {
					$path = "/nso/edit.php?codice={$risultato->fetch(PDO::FETCH_ASSOC)["codice"]}";
				} 
