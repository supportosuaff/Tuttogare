<?
	...
	if (isset($_GET["codice"]) && isset($_GET["codice_gara"])) {
				...
				$inviti = $pdo->go("SELECT * FROM r_inviti_gare WHERE codice_gara = :codice",[":codice"=>$_GET["codice_gara"]]);
				$inviti = $inviti->rowCount();

				$strsql = "SELECT * FROM b_avvisi WHERE codice = :codice";
...
<form name="box" method="post" action="save.php" rel="validate">
					...
					<input type="image" onClick="elimina('<? echo $codice ?>','<? echo "gare/avvisi"; ?>');" src="/img/del.png" title="Elimina">
					<table width="100%">
						<tr>
							...
							<td class="etichetta">Invia PEC</td>
							<td>
								<input type="checkbox" onClick="$('#checkInviaCpv').slideToggle();" name="invia" title="Invia PEC">
								<? if (empty($inviti)) { ?>Invia PEC a partecipanti
									<div style="display:none" id="checkInviaCpv">
										<input type="checkbox" name="invia_cpv" title="Invia a tutti"> Anche a OOEE con CPV compatibili
									</div>
								<? } ?>
							</td>
						</tr>
