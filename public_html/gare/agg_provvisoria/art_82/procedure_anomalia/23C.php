        ...
        if ($ris->rowCount()>0) {
          ...
          if ($valoreE % 2 == 0) {
            if (empty($settings["interpretazione_anomalia"]) || $settings["interpretazione_anomalia"] == "M") {
              $soglia_anomalia = $coef_e - $ribasso_soglia;
            } else if ($settings["interpretazione_anomalia"] == "T") {
              $soglia_anomalia = $coef_e - (($coef_e * $ribasso_soglia) / 100);
            }
          } else {
            if (empty($settings["interpretazione_anomalia"]) || $settings["interpretazione_anomalia"] == "M") {
              $soglia_anomalia = $coef_e + $ribasso_soglia;
          ...
          $msg = "Anomalia calcolata ai sensi dell'allegato II.2 del D.Lgs 36/2023 - Metodo C - Sommatoria: {$somma} - Somma cifre: {$valoreE} - Media: {$media} - Scarto medio: {$scarto_medio} - Variazione: {$ribasso_soglia} - Soglia individuata: {$soglia_anomalia}";
          $msg .=  "\\n";
        } else {
