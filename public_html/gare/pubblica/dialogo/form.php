...
<div id="invito">
      <?
			...
			$strsql .= "JOIN r_partecipanti_dialogo ON r_partecipanti_dialogo.codice_operatore = b_operatori_economici.codice ";
			$strsql .= "JOIN b_bandi_dialogo ON b_bandi_dialogo.codice = r_partecipanti_dialogo.codice_bando ";
			$strsql .= "WHERE b_gruppi.gerarchia > 2 AND b_utenti.attivo = 'S' AND r_partecipanti_dialogo.ammesso = 'S' ";
			$strsql .= "AND (b_bandi_dialogo.annullata = 'N' AND  b_bandi_dialogo.data_apertura < now() ";
			$strsql .= "AND (b_bandi_dialogo.codice_ente = :codice_ente OR b_bandi_dialogo.codice_gestore = :codice_ente) ";
			$strsql .= "AND (b_bandi_dialogo.pubblica = '2' OR b_bandi_dialogo.pubblica = '1') AND b_bandi_dialogo.codice = :codice_derivazione)";
      ...
      <script>
			...
			<div class="clear"></div>
			<input type="hidden" id="indirizzi" name="indirizzi" title="Destinatari" rel="<? echo $rel ?>">
    </div>
<? } ?>
