...
<? if ($record_procedura["invito"] == "S") { ?>
	<div id="invito">
      <?
			...
			$strsql .= "JOIN r_partecipanti_sda ON r_partecipanti_sda.codice_operatore = b_operatori_economici.codice ";
			$strsql .= "JOIN b_bandi_sda ON b_bandi_sda.codice = r_partecipanti_sda.codice_bando ";
			$strsql .= "JOIN r_cpv_operatori_sda ON r_cpv_operatori_sda.codice_bando = b_bandi_sda.codice AND r_cpv_operatori_sda.codice_operatore = b_operatori_economici.codice ";
			$strsql .= "WHERE b_gruppi.gerarchia > 2 AND b_utenti.attivo = 'S' AND r_partecipanti_sda.ammesso = 'S' ";
			$strsql .= "AND (b_bandi_sda.annullata = 'N' AND  b_bandi_sda.data_scadenza > now() ";
			$strsql .= "AND (b_bandi_sda.codice_ente = :codice_ente OR b_bandi_sda.codice_gestore = :codice_ente) ";
			$strsql .= "AND (b_bandi_sda.pubblica = '2' OR b_bandi_sda.pubblica = '1') AND b_bandi_sda.codice = :codice_derivazione)";
				...
				$cont = 0;
					foreach($categorie as $codice) {
						...
						if ($codice != "") {
							...
							$strsql .= "(r_cpv_operatori_sda.codice = :cpv_" . $cont . " ";
							if (strlen($codice)>2) {
								$diff = strlen($codice) - 2;
								for($i=1;$i<=$diff;$i++) {
									$bind[":sub_".$cont."_".$i] = substr($codice,0,$i*-1);
									$strsql .= "OR r_cpv_operatori_sda.codice = :sub_".$cont."_".$i." ";
								}
							}
						$strsql.=") OR ";
					}
