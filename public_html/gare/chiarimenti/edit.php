<?
	...
	if (isset($_GET["codice"]) && isset($_GET["codice_gara"])) {
				...
				$strsql = "SELECT b_quesiti.*, b_operatori_economici.ragione_sociale FROM b_quesiti JOIN b_operatori_economici ON b_quesiti.utente_modifica = b_operatori_economici.codice_utente
									 WHERE b_quesiti.codice = :codice AND b_quesiti.codice_ente = :codice_ente AND b_quesiti.codice_gara = :codice_gara";
				...
				if ($risultato->rowCount() > 0) {
					...
					$bind[":codice"] = $codice;
					$bind[":codice_gara"] = $_GET["codice_gara"];
					$strsql = "SELECT * FROM b_risposte WHERE codice_quesito = :codice AND codice_gara = :codice_gara";
					$risultato = $pdo->bindAndExec($strsql,$bind);
					if ($risultato->rowCount() > 0) {
						$record_risposta = $risultato->fetch(PDO::FETCH_ASSOC);
						$operazione = "UPDATE";
					} else {
						$record_risposta = get_campi("b_risposte");
						$operazione = "INSERT";
					}
?>
		<script>
		...
		<form name="box" method="post" action="save.php" rel="validate">
			...
			<div class="box">
				...
				<div style="padding:5px" id="quesito_originale">
					<? echo $record_quesito["testo"] ?>
				</div>
				<?
			...
			<div style="text-align:right"><strong><?= $record_quesito["ragione_sociale"] ?> - <?= mysql2datetime($record_quesito["timestamp"]) ?></strong>
			</div>
			<br><br>
			...
			<textarea rows='10' class="ckeditor_full" name="testo" cols='80' id="testo" title="Risposta" rel="S;3;0;A"><? echo $record_risposta["testo"]; ?></textarea>
