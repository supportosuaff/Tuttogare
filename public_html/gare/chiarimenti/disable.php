<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"])) {
			...
			$bind = array();
			$bind[":codice"] = $codice;
			$strsql = "SELECT attivo FROM b_quesiti WHERE codice = :codice";
			...
			$strsql = "SELECT codice_gara FROM b_quesiti WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			$record = $risultato->fetch(PDO::FETCH_ASSOC);
			$bind[":attivo"] = $attivo;
			$strsql = "UPDATE b_quesiti SET attivo = :attivo WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			scrivilog("b_quesiti","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);
			
			log_gare($_SESSION["ente"]["codice"],$record["codice_gara"],"UPDATE","Quesito: " . $codice . " - Attivo: " . $attivo);
			?>
