		...
		if ($risultato->rowCount() > 0) {
			...
			if ($print_form) {
				...
				if ($ris_aggiudicatario->rowCount()>0) {
					...
					$editor_tipo = "svincolo_fideiussione";
					...
					if ($risultato->rowCount()>0) {
						...
						$sql_modello = "SELECT * FROM b_modelli_standard WHERE codice = 30"; // Codice 30 - Svincolo fideiussione
					...
					<form name="box" method="post" action="save.php" rel="validate">
						...
						<div style="float:left; width:65%">
							...
							if ($codice_elemento > 0 && $elemento["bozza"] == "N") { ?>
								<input type="button" style="background-color:#C00" class="submit_big" onClick="elimina('<? echo $codice_elemento ?>','gare/svincolo');" src="/img/del.png" value="Rielabora Comunicazione">
								<?
							} else { ?>
								<? if ($codice_elemento > 0 && $elemento["bozza"] == "S") { ?><input type="button" style="background-color:#C00" class="submit_big" onClick="elimina('<? echo $codice_elemento ?>','gare/svincolo');" src="/img/del.png" value="Rielabora Comunicazione"><? } ?>
								<input class="submit_big" type="submit" onClick="$('#bozza').val('S');" value="Salva una bozza">
								<input class="submit_big" type="submit" onClick="$('#bozza').val('N');$('#invia').val('S')" value="Invia comunicazione">
							<? } ?>
						</div>
