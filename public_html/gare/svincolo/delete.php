	...
	if (!$edit) {
		...
		if (isset($_POST["codice"])) {
			...
			if ($risultato->rowCount()>0) {
				...
				log_gare($_SESSION["ente"]["codice"],$record["codice_gara"],"DELETE","Modello svincolo fideiussione");
				?>
	         alert("Rielaborazione effettuata con successo");
	         window.location.href = '/gare/svincolo/index.php?codice=<? echo $record["codice_gara"] ?>';
			   <?
			 } else {
				 ?>
