<?
	...
	if ($edit && !$lock) {
		...
		if ($risultato->rowCount()>0) {
			...
			$documentale["tipo"] = "avviso_exaequo";
			...
			if(! empty($_POST["bozza"]) && $_POST["bozza"] == "N") {
				$sql = "UPDATE b_gare SET stato = 6 WHERE codice = :codice_gara";
				$update_stato = $pdo->bindAndExec($sql,$bind);

				log_gare($_SESSION["ente"]["codice"],$_POST["codice_gara"],"Invio","Avviso di sorteggio");

				$oggetto = "Avviso di sorteggio: " . $record_gara["oggetto"];
				$html= "<html><body>{$_POST["corpo"]}</body></html>";

				$allegato["online"] = 'S';
				$allegato["codice_gara"] = $_POST["codice_gara"];
				$allegato["codice_ente"] = $_SESSION["ente"]["codice"];
				$allegato["nome_file"] = $allegato["codice_gara"] . " - Avviso di sorteggio.".time().".pdf";
				$allegato["titolo"] = "Avviso di sorteggio";
				...
				$avviso["titolo"] = "Avviso di sorteggio - Procedura " . $record_gara["nome_procedura"] . ": " . $record_gara["oggetto"];
				$avviso["testo"] = "Si comunica che &egrave; stato pubblicato l'avviso di sorteggio per la gara in oggetto";
