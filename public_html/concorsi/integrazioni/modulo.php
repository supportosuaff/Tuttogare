<?
	...
	$public = true;
		if ((isset($_GET["cod"]) || isset($_POST["cod"]))&& is_operatore()) {
				...
				$strsql  = "SELECT b_integrazioni_concorsi.*, r_partecipanti_utenti_concorsi.pec, r_partecipanti_utenti_concorsi.partita_iva, r_partecipanti_utenti_concorsi.ragione_sociale, r_integrazioni_concorsi.aperto, r_integrazioni_concorsi.timestamp_trasmissione, r_integrazioni_concorsi.codice_partecipante,r_integrazioni_concorsi.nome_file, b_concorsi.codice_pec, b_concorsi.oggetto, b_concorsi.public_key, b_fasi_concorsi.oggetto AS fase FROM
										...
										JOIN r_partecipanti_utenti_concorsi ON r_partecipanti_utenti_concorsi.codice_partecipante = r_partecipanti_concorsi.codice
										JOIN b_fasi_concorsi ON b_integrazioni_concorsi.codice_fase = b_fasi_concorsi.codice
										WHERE r_integrazioni_concorsi.codice = :codice_integrazione AND r_integrazioni_concorsi.codice_utente = :codice_utente
										AND b_concorsi.annullata = 'N'
										AND (b_concorsi.codice_ente = :codice_ente OR b_concorsi.codice_gestore = :codice_ente)
										AND (b_concorsi.pubblica = '2' OR b_concorsi.pubblica = '1') ";
				...
				?>
<h1>INTEGRAZIONI CONCORSO</h1>
<div class="box"><h2><? echo $integrazione["oggetto"] . " - " . $integrazione["fase"] ?></h2></div>
<?
	$msg = "";
		...
		if ($md5_file == $_POST["md5_file"]) {
			...
			if ($esito == "Verification successful") {
				...
				$msg .= "<ul class=\"firme\">";
					...
					$msg .= "</ul>";
					$msg .= "</li>";
					$path = "/concorsi/" . $integrazione["codice_gara"] . "/" . $integrazione["codice_fase"] . "/integrazioni/".$integrazione["codice"]."/";
					$upload = array();
					...
					if (!$errore) {
						...
						$salva->debug = false;
						$salva->codop = $_SESSION["codice_utente"];
						$salva->nome_tabella = "r_integrazioni_concorsi";
						...
						if ($codice_upload != false) {
							$msg .= "<li>Salvataggio effettuato con successo</li>";
								...
								$mailer->destinatari = $integrazione["pec"];
								$esito = $mailer->send();
	...
	<div class="box">
				<table width="100%">
					...
					if (isset($ris_allegati) && ($ris_allegati->rowCount()>0)) {
						...
						if ($integrazione["timestamp_trasmissione"] > 0) {
							...
							?>
							<div class="box">
								<h2 style="color:#0C0">Trasmissione effettuata: <?= mysql2completedate($integrazione["timestamp_trasmissione"]) ?></h2>
							</div>
							<?
						}
	        	...
	        	<div style="float:left; width:25%">
						...
						<div style="float:left; width:25%">
							...
							<div>Selezionare il file, <? if ($integrazione["data_apertura"]>0) { ?>inserire una chiave personalizzata <? } ?>e cliccare su INVIA</div>
						</div>
						<div class="clear"></div>
				</div>
				<div class="box">
        ...
        <div id="progress_bar" class="big_progress_bar" style="display:none"><div class="progress_bar"></div></div>
			...
			?>
			<a href="/concorsi/integrazioni/view.php?cod=<?= $integrazione["codice_gara"] ?>" class="ritorna_button submit_big" style="background-color:#999;">Ritorna</a>
			<?
		} else {
			echo "<h1>Integrazione inesistente o privilegi insufficienti</h1>";
		}
	} else {
		echo "<h1>Integrazione inesistente</h1>";
	}
