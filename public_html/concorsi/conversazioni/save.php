<?
	...
	$lock = true;
		if (isset($_SESSION["codice_utente"]) && isset($_SESSION["ente"])) {
			$strsql = "SELECT * FROM b_conf_gestione_concorsi WHERE link = '/concorsi/conversazioni/edit.php'";
		...
		if ($edit && !$lock)
	 {
		if (isset($_POST["codice_gara"])) {
			$href = "/concorsi/conversazioni/edit.php?codice=".$_POST["codice_gara"];
			if ((isset($_POST["codice_gara"]) && !empty($_POST["codice_gara"])) && $_POST["testo"] != "") {
				$sql = "SELECT * FROM b_concorsi WHERE codice = :codice_gara ";
				...
				if ($ris_gara->rowCount() > 0) {
					...
					$salva->nome_tabella = "b_messaggi_concorsi";
					$salva->operazione = "INSERT";
					$salva->oggetto = $msg;
					$codice_msg = $salva->save();
					if ($codice_msg > 0) {
						log_concorso($_SESSION["ente"]["codice"],$_POST["codice_gara"],"INSERT","Messaggio interno #" . $codice_msg);
						?>
						...
						$sql = "SELECT b_utenti.* FROM b_utenti JOIN b_gruppi ON b_utenti.gruppo = b_gruppi.codice
										WHERE b_utenti.attivo = 'S' AND b_utenti.codice <> :codice_utente AND (
											...
											(
												SELECT codice_utente FROM b_permessi_concorsi WHERE codice_gara = :codice_gara
											)
										)";
						...
						if ($ris->rowCount() > 0) {
							...
							if (count($indirizzi)>0) {
									...
									$oggetto = "Nuovo messaggio per il concorso: " . $gara["oggetto"];
