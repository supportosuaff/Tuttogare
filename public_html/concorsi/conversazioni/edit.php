<?
	...
	if (isset($_GET["codice"])) {
			...
			<form name="box" method="post" action="/concorsi/conversazioni/save.php" rel="validate">
				<input type="hidden" name="codice_gara" value="<?= $codice ?>">
			<?
				$bind = array();
				$bind[":codice_gara"] = $codice;
				$strsql  = "SELECT b_messaggi_concorsi.*, b_utenti.nome, b_utenti.cognome FROM b_messaggi_concorsi
										JOIN b_utenti ON b_messaggi_concorsi.utente_modifica = b_utenti.codice
										JOIN b_gruppi ON b_utenti.gruppo = b_gruppi.codice
										WHERE b_messaggi_concorsi.codice_gara = :codice_gara
										ORDER BY b_messaggi_concorsi.timestamp";
				...
				if ($numero_msg > 0) {
					while ($msg = $ris_msg->fetch(PDO::FETCH_ASSOC)) {
						...
						if ($msg["utente_modifica"] == $_SESSION["codice_utente"]) {
							...
							$sql_read = "SELECT * FROM r_read_concorsi WHERE utente_modifica = :codice_utente AND codice_messaggio = :codice_messaggio";
							...
							if ($ris_read->rowCount() == 0) {
								...
								$salva->nome_tabella = "r_read_concorsi";
