<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"]) && $_SESSION["gerarchia"]==0) {
			...
			$strsql = "SELECT attivo FROM b_tipo_attivita WHERE codice = :codice";
			...
			$bind[":attivo"] = $attivo;
			$strsql = "UPDATE b_tipo_attivita SET attivo = :attivo WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			scrivilog("b_tipo_attivita","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);
