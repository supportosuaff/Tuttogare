<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"]) && $_SESSION["gerarchia"]==0) {
			...
			$strsql = "SELECT attivo FROM b_tipologie WHERE codice = :codice";
			...
			$bind[":attivo"] = $attivo;
			$strsql = "UPDATE b_tipologie SET attivo = :attivo WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			scrivilog("b_tipologie","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);
