<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"]) && ($_SESSION["gerarchia"] === "0" || $_SESSION["tipo_utente"]== "CON")) {
			...
			$bind = array();
			$bind[":codice"] = $codice;
			$strsql = "SELECT attivo FROM b_categorie_soa WHERE codice = :codice";
			...
			$bind[":attivo"] = $attivo;
			$strsql = "UPDATE b_categorie_soa  SET attivo = :attivo WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			scrivilog("b_categorie_soa ","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);
