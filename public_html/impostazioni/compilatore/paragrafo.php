...
if (isset($paragrafo)) {
	...
	if ($paragrafo["attivo"] == "N") {
		...
		if ($paragrafo["codice_opzione"] != "" || !empty($paragrafo["tipologie"]) || !empty($paragrafo["criteri"]) || $paragrafo["importo_minimo"] != 0 || $paragrafo["importo_massimo"] != 0 || $paragrafo["modalita"] != 0 || $paragrafo["vincoli_soa"] != 0) {
			$colore = "#FC0";
		}
	}
?>
	...
	<div id="paragrafo_content_<?= $id ?>">
		<div class="box">
			...
			<div style="float:right;">
				<button class="btn-round btn-warning" onClick="$('.settings_<?= $id ?>').toggle();return false;" title="Dettagli"><span class="fa fa-search"></span>
				<button class="btn-round btn-primary"  onClick="edit_paragrafo('<? echo $id ?>');return false" title="Modifica"><span class="fa fa-pencil"></span></button>
						...
						<tr>
							...
							<th>Modalita</th><td>
								...
								<tr>
									<th>Criteri</th><td>
											<?
											if (!empty($paragrafo["criteri"])) {
												$paragrafo["criteri"] = ltrim($paragrafo["criteri"],",");
												$sql = "SELECT * FROM b_criteri WHERE eliminato = 'N' AND codice IN (" .$paragrafo["criteri"] .")";
												$ris_criteri = $pdo->bindAndExec($sql,array(":criteri"=>$paragrafo["criteri"]));
												if ($ris_criteri->rowCount()>0) {
													while($criteri = $ris_criteri->fetch(PDO::FETCH_ASSOC)) {
														echo $criteri["criterio"] . "<br>";
													}
												}
											} else {
												?>Tutti<?
										...
										<th>Tipologie</th><td>
												<?
												if (!empty($paragrafo["tipologie"])) {
													$sql = "SELECT * FROM b_tipologie WHERE eliminato = 'N' AND codice IN (" .$paragrafo["tipologie"] .")";
													$ris_tipologie = $pdo->bindAndExec($sql,array(":tipologie"=>$paragrafo["tipologie"]));
													if ($ris_tipologie->rowCount()>0) {
													while($tipologie = $ris_tipologie->fetch(PDO::FETCH_ASSOC)) {
														echo $tipologie["tipologia"] . "<br>";
													}
													}
												} else {
													?>Tutti<?
									...
									case "ricorsivo":
										...
										if ($ris_rico->rowCount() == 1) {
											...
											<div style="border:1px solid #CCC; background-color:#fff; padding:5px; max-height:300px; overflow:auto">
												<? echo $ris_rico->fetch(PDO::FETCH_ASSOC)["contenuto"] ?>
											</div>
											<?
										} break;
									default: ?>
			...
			<button class="aggiungi" onClick="aggiungi('edit_paragrafo.php','#paragrafo_<?= $id ?>','',true);return false;"><span class="fa fa-plus-circle"></span> Aggiungi paragrafo</button>
		</div>
	</div>
	<? } ?>
