<?
	...
	if (!$edit) {
		...
		if (isset($_POST["codice"])) {
			...
			$strsql = "SELECT attivo FROM b_modelli_new WHERE codice = :codice";
			...
			$strsql = "UPDATE b_modelli_new SET attivo = :attivo WHERE codice = :codice";
			$risultato = $pdo->bindAndExec($strsql,$bind);
			$strlog = scrivilog("b_modelli_new","UPDATE",$pdo->getSQL(),$_SESSION["codice_utente"]);
			$risultato = $pdo->query($strlog);
			?>
			if ($("#flag_<? echo $codice ?>").length > 0){
            	$("#flag_<? echo $codice ?>").css('background-color',"<? echo $colore ?>");
            } else {
            	window.location.href="/impostazioni/compilatore/";
            }
			<?
		}
	}
