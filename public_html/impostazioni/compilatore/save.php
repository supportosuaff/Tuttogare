		...
		if (isset($_POST) && ($_SESSION["gerarchia"] === "0" || $_SESSION["tipo_utente"]== "CON")) {
			...
			if ($codice !== false) {
				if (!$duplica) {
						...
						foreach($_POST["paragrafo"] AS $paragrafo) {
							...
							if (!is_numeric($paragrafo["codice"])) {
								...
								if ($paragrafo["ordinamento"] == "") {
									$sql = "SELECT MAX(ordinamento) AS ordinamento FROM b_paragrafi_new WHERE codice_modello = :codice";
							...
							if (isset($paragrafo["criteri"])) $paragrafo["criteri"] = implode(",",$paragrafo["criteri"]);
							if (isset($paragrafo["tipologie"])) $paragrafo["tipologie"] = implode(",",$paragrafo["tipologie"]);

							$salva = new salva();
							$salva->debug = false;
							$salva->codop = $_SESSION["codice_utente"];
							$salva->nome_tabella = "b_paragrafi_new";
					...
					$sql = "SELECT * FROM b_paragrafi_new WHERE codice_modello = :codice_origine AND eliminato = 'N' ORDER BY ordinamento ";
  			...
  			<?
				...
				?>
				window.location.href = '/impostazioni/compilatore/edit.php?codice=<?= $codice ?>';
				<?
			}
		}
	}
