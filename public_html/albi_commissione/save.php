	...
	if (!$edit) {
		...
		if (isset($_POST["operazione"])) {
			if ($_POST["operazione"]=="INSERT") {
				...
				$sql = "SELECT max(id) as id FROM b_albi_commissione WHERE codice_gestore = :codice_ente GROUP BY codice_gestore ";
			...
			if (isset($_POST["cpv"])) {
						if ($_POST["cpv"] != "")  {
							...
							$strsql = "DELETE FROM r_cpv_albi_commissione WHERE codice_bando = :codice_bando ";
							...
							foreach($array_cpv as $cpv) {
								if ($cpv != "") {
									...
									$salva->debug = false;
									$salva->codop = $_SESSION["codice_utente"];
									$salva->nome_tabella = "r_cpv_albi_commissione";
			...
			?>
			if (confirm("Passare alla gestione degli iscritti?")) {
				window.location.href = '/albi_commissione/iscritti/index.php?codice=<?= $codice_bando ?>';
			} else {
