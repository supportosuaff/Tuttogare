...
/*##################################################################################################################*\
   ...
   Non è necessario che le stringhe siano maiuscole, mentre la DATA_DI_NASCITA DEVE essere nel formato "ggmmaaaa";
   ...
   Questo programma è Free Software, by Emiliano <AlberT> Gabrielli.
   ...
   Questo programma è stato possibile grazie ai sorgenti di kodicefiscale di Catuzzi Samuele <samuele.c@yahoo.it> !
   Un ringraziamento va a Valter Sini che mi ha segnalato un bug nel calcolo del codice per cognomi come il suo. :-)
   Grazie agli amici di php-it@ziobudda.net è stata semplificata la procedura di controllo della data con l'uso
   di checkdate().
...
function levastranezze($stringa){
 ...
 $stringa=str_replace("[","",$stringa);
 $stringa=str_replace("]","",$stringa);
 $stringa=ereg_replace("[\\\"!?^$|£%&)(=`,;:.+*/<>@#°§ç]","",$stringa);
...
function cercacitta($comune, $pr) {
  ...
  $comune = strtoupper( substr($comune,0,20) );
  $pr = strtoupper( substr($pr,0,2) );
  $comune = str_replace(" ", "", $comune);
  $pr = str_replace(" ", "", $pr);
  $dati = file($CFconf["thisdir"].sceglifile($comune));              // CARICA I DATI IN UN ARRAY

  for ($i=0;$i<sizeof($dati);$i++) {                                 // CERCA LA LOCAZIONE GIUSTA
    $daticitta = split(":", $dati[$i], 3) ;                          // ED ESTRAE I DATI
    $daticitta[0] = str_replace(" ", "", $daticitta[0]);
    if ($daticitta[0]==$comune) {
      $checkpr = split(":", $dati[$i+1], 3);
      $next = str_replace(" ", "", $checkpr[0]);

      if(($pr == "" && $next != $comune) || $pr==$daticitta[1]) {
        $PIPPO = "PiPpO";
        break;
       }
      else if($next == $comune && $pr == "") {
        $error = "Bisogna specificare la provincia";
        break;
       }
    }
  }
  $ritorna = ($PIPPO=="PiPpO") ? $daticitta : $error;
  return $ritorna;
}
...
//   Ora calcolo il carattere del mese
 ...
 for ($i=0; $i<=14; $i+=2) {                                         // ciclo per i caratteri dispari
   if ( strchr($numeri,substr($codicefiscale,$i,1))==false )  {
     ...
     switch ( substr($codicefiscale,$i,1) )  {                       // è un numero
...
/*
 echo  AlberT_CodFis("gabrielli","emiliano","01031975","m","L'Aquilà")."<br>";
